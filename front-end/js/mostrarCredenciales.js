...
function actualizarCredenciales() {
    ...
    $.ajax({
        ...
        beforeSend: function () {
            btnActualizar.innerHTML = '<i class="fas fa-user-edit"></i> Actualizando...';
        },
        success: function (dato) {
            btnActualizar.innerHTML = '<i class="fas fa-user-edit"></i> Actualizar';
            mostrarCredenciales();
            limpiarCampos();
            if (dato.result == 1) {
                Swal.fire({
...
$(document).on('submit', '#FormCredenciales', function (e) {
    ...
    if ($estadoFormulario == true) {
      ...
      if($('#txtClave').val() == $('#txtClaveActual').val()){
        ...
        if($('#txtNewClave').val() == $('#txtConfirClave')){
